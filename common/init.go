...
func initConstantEnv() {
	...
	// STREAM_SCANNER_MAX_BUFFER_MB controls the maximum size of one SSE line;
	// 0 means unlimited (bounded only by the platform's addressable memory).
	constant.StreamScannerMaxBufferMB = GetEnvOrDefault("STREAM_SCANNER_MAX_BUFFER_MB", 0)
	// MaxRequestBodyMB 请求体最大大小（解压后）；0 表示不限制。
	constant.MaxRequestBodyMB = GetEnvOrDefault("MAX_REQUEST_BODY_MB", 0)
