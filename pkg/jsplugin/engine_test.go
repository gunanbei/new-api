...
func TestCompileRejectsSandboxEscapeGlobals(t *testing.T) {
	for _, source := range []string{
		`export function run() { return fetch("https://example.com"); }`,
		`export function run() { return process.env.SECRET; }`,
		`export function run() { return require("fs"); }`,
		`export function run() { return Promise.resolve(1); }`,
		`export function run() { return eval("1"); }`,
	} {
		_, err := Compile(source, Options{Key: "escape"})
		require.ErrorContains(t, err, "unsupported plugin syntax")
	}
}

func TestEngineEnforcesJSONInputAndOutputBounds(t *testing.T) {
	engine, err := Compile(`export function echo(value) { return value; }`, Options{Key: "bounds", MaxInputBytes: 8, MaxOutputBytes: 8})
	require.NoError(t, err)
	_, err = engine.Call(context.Background(), "echo", "0123456789")
	require.ErrorContains(t, err, "input exceeds")
	_, err = engine.Call(context.Background(), "echo", "ok")
	require.NoError(t, err)
	_, err = engine.Call(context.Background(), "echo", "123456789")
	require.ErrorContains(t, err, "input exceeds")
}
