...
var forbiddenSyntax = regexp.MustCompile(`(?m)(^|[^A-Za-z0-9_$])(async|await|import|fetch|require|Promise|process|fs|eval|Function)([^A-Za-z0-9_$]|$)`)
...
func Compile(source string, options Options) (*Engine, error) {
	if match := forbiddenSyntax.FindString(sourceWithoutCommentsAndStrings(source)); match != "" {
		return nil, fmt.Errorf("unsupported plugin syntax %q: plugins must be synchronous and cannot access network, filesystem, process, or imports", strings.TrimSpace(match))
	}
...
func (e *Engine) call(
	...
	if e.maxInputBytes > 0 {
		encoded, marshalErr := common.Marshal(args)
		if marshalErr != nil {
			return nil, fmt.Errorf("plugin hook %s input is not JSON-compatible", hookName)
		}
		if len(encoded) > e.maxInputBytes {
			return nil, fmt.Errorf("plugin hook %s input exceeds %d bytes", hookName, e.maxInputBytes)
		}
	}
	...
	if e.maxOutputBytes > 0 {
		...
		if marshalErr != nil {
			return nil, fmt.Errorf("plugin hook %s output is not JSON-compatible", hookName)
		}
		if len(encoded) > e.maxOutputBytes {
			return nil, fmt.Errorf("plugin hook %s output exceeds %d bytes", hookName, e.maxOutputBytes)
		}
	}
	return result, nil
}
