...
现有 `image_playground.model_registry` 仅描述旧在线生图的展示和适配器类型，且作为全局 Option 存储。创作台模型目录独立配置，不读取、迁移或回写该 Option。
...
| 旧图片注册表 | 保持旧在线生图的独立配置来源，不与创作台互相转换。 |
...
- 管理员从现有启用模型中选择模型名，并以固定供应商、协议和能力预设创建目录配置。
- 旧 Option 与旧在线生图保持独立，不提供迁移或回写。
