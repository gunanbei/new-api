...
function normalizeHttpProtocol(
  ...
  return String(value || '')
    .trim()
    .toLowerCase() === HTTP_PROTOCOL_HTTP1
    ? HTTP_PROTOCOL_HTTP1
    : HTTP_PROTOCOL_AUTO
}
...
function normalizeHttp2ConnectionShards(
  ...
  if (!Number.isFinite(value) || value == null || value < 1) return 1
  return Math.min(value, MAX_HTTP2_CONNECTION_SHARDS)
}
...
export const channelFormSchema = z
  .object({
    ...
    http2_connection_shards: z.number().optional(),
