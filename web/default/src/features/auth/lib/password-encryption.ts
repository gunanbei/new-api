...
async function rsaOaepEncrypt(
  ...
  if (typeof globalThis.crypto?.subtle !== 'undefined') {
    try {
      ...
      // Fall back for browsers exposing SubtleCrypto without RSA-OAEP support.
    }
  }
  // Keep the legacy fallback lazy so builds that only use Web Crypto do not
  // require bundling forge in the initial authentication chunk.
  // eslint-disable-next-line no-eval
  const forge = (await eval(
    'import("node-forge")'
  )) as typeof import('node-forge')
...
async function getPasswordEncryptionKey(): Promise<PasswordEncryptionKey> {
  const now = Date.now()
  if (cachedKey && now - cachedAt < KEY_CACHE_TTL_MS) return cachedKey
